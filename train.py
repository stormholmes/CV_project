...
def train(args):
    ...
    criterion = SetCriterion(0, matcher, {"loss_giou": args.giou_loss_coef}, ["bboxes", "ce", "focal", "iou_aware"],
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for img, bboxes, img_name, gt_bboxes, _ in train_loader:
            ...
            for idx in range(img.shape[0]):
                ...
                losses.append(l['loss_giou'] + l["loss_l2"] + + l["loss_bbox"] + l["loss_centerness_focal"]) # + l["loss_iou_aware"])
            loss = sum(losses)
        ...
        with torch.no_grad():
            for img, bboxes, img_name, gt_bboxes, _ in val_loader:
                ...
                for idx in range(img.shape[0]):
                    ...
                    if 'loss_distill' in l:
                        distill_losses.append(l['loss_distill'])
                loss = sum(losses)
                if distill_losses:
                    loss += sum(distill_losses)
