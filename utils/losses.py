...
class SetCriterion(nn.Module):
    ...
    def loss_iou_aware(self, outputs, targets, indices, num_boxes, centerness, centerness_gt, mask):
            """让centerness预测与IoU相关"""
            if len(indices[0][0]) == 0:  # 没有匹配的预测
                return {'loss_iou_aware': torch.tensor(0.0, device=centerness.device)}
            ...
            # 计算IoU
            iou_result = box_ops.box_iou(src_boxes, target_boxes)
            # 如果返回的是元组，通常第一个元素是iou矩阵
            if isinstance(iou_result, tuple):
                ious = iou_result[0].diag()  # 取元组的第一个元素
            else:
                ious = iou_result.diag()  # 直接使用张量
            
            # 获取对应的centerness预测位置
            batch_idx, src_idx = idx
            pred_centerness = centerness[batch_idx, src_idx // centerness.shape[2], src_idx % centerness.shape[2]]
            
            # IoU-aware损失
            iou_loss = F.mse_loss(pred_centerness, ious)
            
            return {'loss_iou_aware': iou_loss}

    def loss_centerness_focal(self, outputs, targets, indices, num_boxes, centerness, centerness_gt, mask):
            """使用Focal Loss来处理centerness的类别不平衡"""
            pred = centerness[mask > 0]
            target = centerness_gt[mask > 0]
            
            if len(pred) == 0:
                return {'loss_centerness_focal': torch.tensor(0.0, device=centerness.device)}
            
            # Binary Focal Loss
            BCE = F.binary_cross_entropy_with_logits(pred, target, reduction='none')
            p_t = torch.exp(-BCE)
            focal_loss = (self.focal_alpha * (1 - p_t) ** 2 * BCE)
            
            return {'loss_centerness_focal': focal_loss.mean()}

    def get_loss(self, loss, outputs, targets, indices, num_boxes, centerness, centerness_gt, mask, **kwargs):
        loss_map = {
            'bboxes': self.loss_boxes,
            'ce': self.ce_loss,
            'iou_aware': self.loss_iou_aware,
            'focal': self.loss_centerness_focal
        }
